...
class Movie_Theatre(Resource):
    ...
    def get(self,movie_id):
        ...
        if 'admin' or 'user' in  decoded_token['role']:
            ...
            theat = TheatreMovie.query.filter_by(movie_id=movie_id).all()
            final = []
            for x in theat:
                ...
                k = Theatre.query.filter_by(the_id=theID).first()
          
                p = Movie.query.filter_by(movie_id=movie_id).first()
             
                d["theatre_name"] = k.the_name
                d["place"]=k.place
                d['the_id'] = theID
